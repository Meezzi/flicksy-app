...
void main() {
  ...
  late RemoteMovieService remoteMovieService;
  late RemoteMovieRepository remoteMovieRepository;

  setUp(() {
    mockDio = MockDio();
    remoteMovieService = RemoteMovieService(dio: mockDio);
    remoteMovieRepository = RemoteMovieRepository(remoteMovieService);
  });

  group('RemoteMovieRepository', () {
    test('fetchMovieDetail', () async {
      ...
      final result = await remoteMovieRepository.fetchMovieDetail(278);

      expect(result?.title, '살인에 관한 짧은 필름');
      expect(result?.genres.first, '범죄');
      expect(result?.releaseDate.year, 1988);
    });
    ...
    test('fetchNowPlayingMovies', () async {
      ...
      final result = await remoteMovieRepository.fetchNowPlayingMovies();

      expect(result?.first.id, 278);
      expect(result?.first.posterPath, '/oAt6OtpwYCdJI76AVtVKW1eorYx.jpg');
    });
    ...
    test('fetchPopularMovies', () async {
      ...
      final result = await remoteMovieRepository.fetchPopularMovies();
    ...
    test('fetchTopRatedMovies', () async {
      ...
      final result = await remoteMovieRepository.fetchTopRatedMovies();
    ...
    test('fetchUpcomingMovies', () async {
      ...
      final result = await remoteMovieRepository.fetchUpcomingMovies();
